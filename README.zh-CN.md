...
用户不需要理解“主控”或“专项对话”等内部概念。要明确采用并持久化 Project Harness，可以直接点选它或说：

```text
使用 project-harness 管理这个项目。
```

明确采用代表允许 Agent 检查项目并写入最小绑定，而不是只在当前对话里临时启用。Agent 会保留已有 `AGENTS.md` 和项目文档，只创建缺失内容，验证结果，并返回 `ADOPTED`、`ALREADY_ADOPTED` 或 `BLOCKED`。

采用后，任务和决策记录主要供后续 Agent 使用。Agent 自动分配 `TASK-0001` 和 `DEC-0001` 格式的编号、保留历史、更新状态，并在创建新任务前检查是否存在可以继续、复用或需要处理冲突的相关工作；用户不需要手动维护。
